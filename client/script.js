...
function getMessage(m) {
    ...
    switch (action) {
        ...
        case 'setCardColour':
            const o = $("#" + data.id).children('.card-image:first');
            o.attr("src", "images/" + data.colour + "-card.png");
            break;
...
function drawNewCard(id, text, x, y, rot, colour, sticker, animationspeed) {
    ...
    var h = '<div id="' + id + '" class="card ' + colour +
        ...
        'deg);\
	  ">\
	  <div class="card-config-icons">\
      <img class="card-colour-icon card-colour-yellow" src="images/icons/token/yellow.png"/>\
      <img class="card-colour-icon card-colour-blue" src="images/icons/token/blue.png"/>\
      <img class="card-colour-icon card-colour-green" src="images/icons/token/green.png"/>\
      <img class="card-colour-icon card-colour-pink" src="images/icons/token/pink.png"/>\
      <img class="card-colour-icon card-colour-white" src="images/icons/token/white.png"/>\
    </div>\
    <img src="images/icons/token/Cicon.png" class="card-icon conf-card-icon" />\
    ...
    card.hover(
        function() {
            ...
            $(this).children('.card-conf-icon').fadeIn(10);
        },
        function() {
            $(this).removeClass('hover');
            $(this).children('.card-icon').fadeOut(150);
            $(this).children('.card-conf-icon').fadeOut(150);
            $(this).find('.card-config-icons:first').fadeOut(10);
        }
    );
    ...
    card.find('.card-colour-icon').hover(
      function() {
        $(this).addClass('card-colour-icon-hover');
      },
      function() {
        $(this).removeClass('card-colour-icon-hover');
      }
    );
    ...
    card.children('.conf-card-icon').click(
      function() {
        $('#'+id).find('.card-config-icons:first').fadeIn(10);
      }
    );

    ['yellow', 'blue', 'green', 'pink', 'white'].forEach( (col) => {
      const sel = '.card-colour-' + col;
      card.find(sel).click(function(){
        onCardChangeImage(id, col);
      });
    });
...
function onShowConfig(id) {
  $('#'+id).children('.card-config-icons')
}

function onCardChangeImage(id, colour) {
  try {
    const a = 'setCardColour';
    const d = {
      id: id,
      colour: colour
    };
    ...
    console.log('card ' + id + ' set img src to ' + src);
  } catch(e){
    ;
  }
}
